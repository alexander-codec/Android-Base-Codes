    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (StringUtils.isNotEmpty(defaultStr)) {
            ...
            if (defaultStr.length() <= maxlength) {
                numberTips.setText("Também pode introduzir" + (maxlength - defaultStr.length()) + "palavras  ("
    ...
    TextWatcher mTextWatcher = new TextWatcher() {
                                 ...
                                 public void afterTextChanged(Editable s) {
                                     ...
                                     if (s.toString().length() > maxlength) {
                                         toast("Insira o número de palavras que você excedeu o limite！", Toast.LENGTH_LONG);
                                         s.delete(editStart - 1, editEnd);
                                         int tempSelection = editStart;
                                         contentView.setText(s);
                                         contentView.setSelection(tempSelection);
                                     }
                                     numberTips.setText("Também pode introduzir"
                                                        + (maxlength - s.toString().length())
                                                        + "palavras  (" + s.toString().length() + "/"
                                                        + maxlength + ")");
                                 }
                             };
}
